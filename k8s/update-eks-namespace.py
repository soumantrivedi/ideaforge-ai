...
def update_manifests(eks_dir, namespace, image_tag):
    ...
    for yaml_file in eks_path.glob("*.yaml"):
        try:
            ...
            content = re.sub(
                r'namespace: ideaforge-ai',
                f'namespace: {namespace}',
                content
            )
            
            # Update name fields (only when it's exactly "ideaforge-ai", not part of a longer name)
            content = re.sub(
                r'name: ideaforge-ai(\s|$)',
                f'name: {namespace}\\1',
                content
            )
            
            # Update namespace name in Namespace resource metadata
            content = re.sub(
                r'(\s+)name: ideaforge-ai(\s*#.*)?$',
                f'\\1name: {namespace}\\2',
                content,
                flags=re.MULTILINE
            )
