        ...
        if rag_agent and product_id:
            try:
                ...
                if design_form_data:
                    ...
                    key_fields = ["user_experience", "target_users", "key_features", "design_style", "branding", "tech_stack"]
                ...
                kb_query = " ".join(kb_query_parts) if kb_query_parts else f"Application development and architecture for product {product_id}"
                ...
                if knowledge_results:
                    ...
                    if kb_summaries:
                        ...
                        logger.info("lovable_prompt_knowledge_base_included",
                                  product_id=product_id,
                                  results_count=len(knowledge_results),
                                  included_count=len(kb_summaries))
            except Exception as e:
                logger.warning("lovable_prompt_kb_retrieval_failed",
                             error=str(e),
                             product_id=product_id)
